...
macro_rules! primitive {
    ($(#[$attrs:meta])* $atomic_type:ident, $value_type:ident) => {
        ...
        impl AtomicablePrimitivePriv for $value_type {
            type Atomic = crate::$atomic_type;
            const IS_ALWAYS_LOCK_FREE: bool = crate::$atomic_type::is_always_lock_free();
            #[inline]
            fn default() -> Self {
            ...
            fn new(v: Self) -> Self::Atomic {
                crate::$atomic_type::new(v)
            }
            #[inline]
            fn is_lock_free() -> bool {
                crate::$atomic_type::is_lock_free()
            }
            #[inline]
...
impl<T> AtomicablePrimitivePriv for *mut T {
    type Atomic = crate::AtomicPtr<T>;
    const IS_ALWAYS_LOCK_FREE: bool = crate::AtomicPtr::<T>::is_always_lock_free();
    #[inline]
    fn default() -> Self {
        ptr::null_mut()
    }
    #[inline]
    fn new(v: Self) -> Self::Atomic {
        crate::AtomicPtr::<T>::new(v)
    }
    #[inline]
    fn is_lock_free() -> bool {
        crate::AtomicPtr::<T>::is_lock_free()
    }
    #[inline]
    fn get_mut(a: &mut Self::Atomic) -> &mut Self {
        a.get_mut()
