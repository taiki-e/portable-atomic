...
| arm         | linux                | getauxval       | all      | Only enabled by default on `*-linux-gnu*` and `*-linux-{musl,ohos,uclibc}*` with dynamic linking enabled (musl is static linking by default). (dlsym is used by default if needed for compatibility with older versions) |
| arm         | android              | getauxval       | all      | Enabled by default (dlsym is used by default for compatibility with older versions) |
