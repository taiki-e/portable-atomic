...
- spinning-based waiting otherwise (we always used this in portable-atomic 1.15.0 and earlier).
...
- It seems there is no stabilized interface:
  - Redox:
    - Not in libc crate and it seems that the interface is not stabilized?
  ...
  - Motor: futex_wait/futex_wake
    - Not in libc crate and it seems syscalls need to be made via VDSO vtable, for which ABI stability may not guaranteed.
        ...
        pub(crate) mod ffi {
            ...
            cfg_sel!({
                ...
                ))]
                {
                    ...
                    pub(crate) unsafe fn futex_syscall3(
                        ...
                        unsafe {
                            ...
                            // x86_32 has fast syscall using __kernel_vsyscall, but is complex,
                            // needs access to auxv (usually exposed via getauxval), and futex
                            // syscalls are called only in the slow path, so we don't use it here.
                    ...
                    #[inline]
                    pub(crate) unsafe fn futex_syscall4(
                        number: c_long,
                        u_addr: *mut u32,
                        op: c_int,
                        val: u32,
                ...
                #[cfg(else)]
                {
                    ...
                    pub(crate) use self::{syscall as futex_syscall3, syscall as futex_syscall4};
                }
            });
        }
