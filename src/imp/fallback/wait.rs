...
mod os {
    ...
    mod ffi {
        ...
        sys_const!({
            ...
            // https://github.com/torvalds/linux/blob/v6.19/include/uapi/asm-generic/unistd.h#L751
            // https://github.com/torvalds/linux/blob/v6.19/arch/arc/include/uapi/asm/unistd.h
            // https://github.com/torvalds/linux/blob/v6.19/arch/arc/kernel/Makefile.syscalls
            ...
            // https://github.com/torvalds/linux/blob/v6.19/arch/csky/kernel/Makefile.syscalls
            // https://github.com/torvalds/linux/blob/v6.19/arch/hexagon/include/uapi/asm/unistd.h
            // https://github.com/torvalds/linux/blob/v6.19/arch/hexagon/kernel/Makefile.syscalls
            // https://github.com/torvalds/linux/blob/v6.19/arch/loongarch/include/uapi/asm/unistd.h
            // https://github.com/torvalds/linux/blob/v6.19/arch/nios2/include/uapi/asm/unistd.h
            // https://github.com/torvalds/linux/blob/v6.19/arch/openrisc/include/uapi/asm/unistd.h
            // https://github.com/torvalds/linux/blob/v6.19/arch/openrisc/kernel/Makefile.syscalls
            ...
            // https://github.com/torvalds/linux/blob/v6.19/arch/loongarch/kernel/Makefile.syscalls
            // https://github.com/torvalds/linux/blob/v6.19/arch/riscv/kernel/Makefile.syscalls
            ...
            // https://github.com/linux-ia64/linux-ia64/blob/v6.19-epic3/arch/ia64/kernel/syscalls/syscall.tbl#L218
            #[cfg(target_arch = "ia64")]
            pub(crate) const __NR_futex: c_long = 206;
...
mod os {
    ...
    pub(crate) fn notify32_all(a: *const AtomicU32) {
        unsafe {
            ffi::umtx_wakeup(a as *const i32, i32::MAX);
        }
    }
...
mod os {
    ...
    mod ffi {
        ...
        sys_const!({
            // https://github.com/redox-os/syscall/blob/0.7.1/src/number.rs#L54
            pub(crate) const SYS_FUTEX: usize = 240;

            // https://github.com/redox-os/syscall/blob/0.7.1/src/flag.rs#L60
            #[allow(dead_code)]
        ...
        pub(crate) unsafe fn syscall5(
            ...
            let ret: usize;
            unsafe {
                // https://github.com/redox-os/syscall/blob/0.7.1/src/arch/aarch64.rs
                ...
                // https://github.com/redox-os/syscall/blob/0.7.1/src/arch/riscv64.rs
                ...
                // https://github.com/redox-os/syscall/blob/0.7.1/src/arch/x86.rs
                #[cfg(target_arch = "x86")]
                asm!(
                    "xchg esi, {e}
                    int 0x80
                ...
                // https://github.com/redox-os/syscall/blob/0.7.1/src/arch/x86_64.rs
                #[cfg(target_arch = "x86_64")]
                asm!(
                    "syscall",
                    inout("rax") a => ret,
...
mod os {
    ...
    mod ffi {
        ...
        dlsym!(
            pub(crate) unsafe extern "C" fn os_sync_wake_by_address_all(
                addr: *mut c_void,
                size: c_size_t,
                flags: u32,
            ) -> c_int;
        ...
        dlsym!(
            pub(crate) unsafe extern "C" fn __ulock_wait(
                operation: u32,
                addr: *mut c_void,
                value: u64,
                timeout: u32,
        ...
        dlsym!(
            pub(crate) unsafe extern "C" fn __ulock_wake(
                operation: u32,
                addr: *mut c_void,
                wake_value: u64,
            ) -> c_int;
    ...
    macro_rules! futex {
        (
            ...
            pub(crate) fn $wait(a: &$atomic, expected: $val) {
                ...
                } else if let Some(wait) = ffi::__ulock_wait() {
                    unsafe {
                        wait(
            ...
            pub(crate) fn $notify_one(a: *const $atomic) {
                ...
                } else if let Some(wake) = ffi::__ulock_wake() {
                    loop {
                        unsafe {
                            let r = wake(
                                match mem::size_of::<$atomic>() {
            ...
            #[inline]
            pub(crate) fn $notify_all(a: *const $atomic) {
                if let Some(wake) = ffi::os_sync_wake_by_address_all() {
                    unsafe {
                        wake(
                            a as *mut ffi::c_void,
                            mem::size_of::<$atomic>(),
...
mod os {
    ...
    mod ffi {
        ...
        // TODO: https://github.com/rust-lang/rust/blob/1.93.0/library/std/src/sys/pal/windows/c.rs#L161
        #[cfg(all(
            target_env = "msvc",
            not(any(target_vendor = "win7", target_family = "rust9x")),
        ))]
        ...
        dlsym!(
            #[link(name = "api-ms-win-core-synch-l1-2-0")]
            pub(crate) unsafe extern "system" fn WakeByAddressAll(address: *const c_void);
        );
    }
    ...
    macro_rules! futex {
        (
            ...
            pub(crate) fn $notify_all(a: *const $atomic) {
                ...
                {
                    if let Some(wake) = ffi::WakeByAddressAll() {
                        unsafe {
                            wake(a as *const c_void);
                        }
                    } else {
...
mod no_futex64 {
    #[cfg(not(miri))]
    use core::{sync::atomic::{AtomicU32, AtomicU64}, u32};
...
#[cfg(not(any(
    target_os = "freebsd",
    all(windows, target_env = "msvc", not(any(target_vendor = "win7", target_family = "rust9x")))
)))]
#[allow(dead_code)]
mod no_futex {
