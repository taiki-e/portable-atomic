...
macro_rules! dispatch_impl {
    ($self:expr, $([$($generics:tt)*])? $atomic_type:ident, $atomic:expr, $($fallback:tt)*) => {
        {
            ...
            if addr >= EXTERNAL_DATA_BUS_ADDRESS_RANGE.start && addr < EXTERNAL_DATA_BUS_ADDRESS_RANGE.end {
                ...
                {
                    let atomic = unsafe { ::core::mem::transmute::<&Self, &core::sync::atomic::$atomic_type $(<$($generics)*>)?>($self) };
                    return atomic.$($fallback)*;
                }
            }
        }
