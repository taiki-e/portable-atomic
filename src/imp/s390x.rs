...
#[inline]
pub(crate) fn extract_cc0_from_cc01(r: i64) -> bool {
    (r >> 28) & 1 == 0
}
// Extracts and checks condition code 1 from condition code 0-1.
#[allow(dead_code)]
#[inline]
fn extract_cc1_from_cc01(r: i64) -> bool {
    (r >> 28) & 1 != 0
}
...
cfg_core_atomic!({
    ...
    items!({
        use super::core_atomic::{
            AtomicI32, AtomicI64, AtomicIsize, AtomicU32, AtomicU64, AtomicUsize,
        };
            ...
            ($atomic_type:ident) => {
                impl $atomic_type {
                    ...
                    pub(crate) fn not(&self, _order: Ordering) {
                        ...
                        // SAFETY: any data races are prevented by atomic intrinsics and the raw
                        // pointer passed in is valid because we got it from a reference.
                        unsafe { atomic_rmw_i!("x", dst, "255") }
                    }
                }
            };
        }
        macro_rules! atomic_not_fallback {
            ($atomic_type:ident) => {
                impl $atomic_type {
                    #[inline]
        ...
        atomic_not!(AtomicI8);
        atomic_not!(AtomicU8);
        atomic_not_fallback!(AtomicI16);
        atomic_not_fallback!(AtomicU16);
        atomic_not_fallback!(AtomicI32);
        atomic_not_fallback!(AtomicU32);
        atomic_not_fallback!(AtomicI64);
        atomic_not_fallback!(AtomicU64);
        atomic_not_fallback!(AtomicIsize);
        atomic_not_fallback!(AtomicUsize);
        ...
        impl AtomicU8 {
            ...
            pub(crate) fn fetch_xor_bool(&self, val: bool) -> bool {
                ...
                if val {
                    ...
                    // extract_cc0_from_cc01 is not good here...
                    extract_cc0(r)
                } else {
                    // SAFETY: any data races are prevented by atomic intrinsics and the raw
                    // pointer passed in is valid because we got it from a reference.
                    let r = unsafe { atomic_rmw_i_ipm!("x", dst, "0") };
                    extract_cc1_from_cc01(r)
                }
            }
            #[inline]
