...
// https://developer.arm.com/documentation/ddi0602/2024-12/SIMD-FP-Instructions/STFMAX--STFMAXL--Floating-point-atomic-maximum-in-memory--without-return-
// https://developer.arm.com/documentation/ddi0602/2024-12/SIMD-FP-Instructions/STFMIN--STFMINL--Floating-point-atomic-minimum-in-memory--without-return-
    ...
    ($atomic_type:ident, $float_type:ident, $modifier:tt, $inst_modifier:tt) => {
        impl $atomic_type {
            ...
            pub(crate) fn fetch_max(&self, val: $float_type, order: Ordering) -> $float_type {
                ...
                // pointer passed in is valid because we got it from a reference.
                //
                // Refs: https://developer.arm.com/documentation/ddi0602/2024-12/SIMD-FP-Instructions/LDFMAX--LDFMAXA--LDFMAXAL--LDFMAXL--Floating-point-atomic-maximum-in-memory-
                unsafe {
                    #[cfg(not(portable_atomic_pre_llvm_20))]
                    macro_rules! max {
                        ($acquire:tt, $release:tt, $fence:tt) => {
                            asm!(
                                start_lsfe!(),
                                concat!("ldfmax", $acquire, $release, " {out:", $modifier, "}, {val:", $modifier, "}, [{dst}]"),
                                $fence,
                                dst = in(reg) ptr_reg!(dst),
                                val = in(vreg) val,
                                out = out(vreg) out,
                    ...
                    macro_rules! max {
                        ($order:tt, $fence:tt) => {
                            asm!(
                                // ldfmax{,a,l,al} {h,s,d}0, {h,s,d}1, [x2]
                                concat!(".inst 0x", $inst_modifier, "c", $order, "04041"),
                                $fence,
                                in("x2") ptr_reg!(dst),
                                in("v1") val,
                                out("v0") out,
            ...
            pub(crate) fn fetch_min(&self, val: $float_type, order: Ordering) -> $float_type {
                ...
                // pointer passed in is valid because we got it from a reference.
                //
                // Refs: https://developer.arm.com/documentation/ddi0602/2024-12/SIMD-FP-Instructions/LDFMIN--LDFMINA--LDFMINAL--LDFMINL--Floating-point-atomic-minimum-in-memory-
                unsafe {
                    #[cfg(not(portable_atomic_pre_llvm_20))]
                    macro_rules! min {
                        ($acquire:tt, $release:tt, $fence:tt) => {
                            asm!(
                                start_lsfe!(),
                                concat!("ldfmin", $acquire, $release, " {out:", $modifier, "}, {val:", $modifier, "}, [{dst}]"),
                                $fence,
                                dst = in(reg) ptr_reg!(dst),
                                val = in(vreg) val,
                                out = out(vreg) out,
                    ...
                    macro_rules! min {
                        ($order:tt, $fence:tt) => {
                            asm!(
                                // ldfmin{,a,l,al} {h,s,d}0, {h,s,d}1, [x2]
                                concat!(".inst 0x", $inst_modifier, "c", $order, "05041"),
                                $fence,
                                in("x2") ptr_reg!(dst),
                                in("v1") val,
                                out("v0") out,
