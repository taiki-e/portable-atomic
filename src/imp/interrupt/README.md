...
- On MSP430 additionally: {8,16}-bit `add,sub,and,or,xor,not`, `AtomicBool::fetch_{and,xor,not}`, `AtomicBool::fetch_nand(true)`, `AtomicBool::{swap,fetch_or}(false)`, `AtomicBool::compare_exchange{,_weak}` other than `AtomicBool::compare_exchange{,_weak}(false,true)`
