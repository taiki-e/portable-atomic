...
Generated asm:
- armv7 https://godbolt.org/z/s65YoPP8z
*/
...
macro_rules! debug_assert_v6 {
    () => {
        #[cfg(not(any(target_feature = "v6", portable_atomic_target_feature = "v6")))]
        {
            debug_assert!(detect::detect().has_v6() && !detect::detect().has_v7());
        }
    };
}
...
unsafe fn atomic_load(src: *mut u64, order: Ordering) -> u64 {
    ...
    // and we've checked if v7/v6/kuser_cmpxchg64 is available.
    unsafe {
        match order {
            Ordering::Relaxed => {
...
unsafe fn atomic_store(dst: *mut u64, val: u64, order: Ordering) {
    fn_alias! {
        ...
        // TODO: v6 and v7 relaxed loads are the same.
...
unsafe fn atomic_compare_exchange(
    ...
    let (prev, ok) = unsafe {
        fn_alias! {
            ...
            v6_relaxed_fn = atomic_compare_exchange_v6(Ordering::Relaxed, Ordering::Relaxed);
            v6_acquire_fn = atomic_compare_exchange_v6(Ordering::Acquire, Ordering::Acquire);
            v6_release_fn = atomic_compare_exchange_v6(Ordering::Release, Ordering::Relaxed);
            // AcqRel and SeqCst RMWs are equivalent.
            v6_seqcst_fn = atomic_compare_exchange_v6(Ordering::SeqCst, Ordering::SeqCst);
        }
        #[inline(never)]
...
#[inline]
unsafe fn atomic_compare_exchange_v6(
    dst: *mut u64,
    old: u64,
    new: u64,
    success: Ordering,
    ...
    unsafe {
        ...
        match (success, failure) {
            ...
            (Relaxed, Acquire | SeqCst) => cmpxchg_store_relaxed!("", dmb_cp15!()),
            (Acquire, Relaxed) => cmpxchg_store_relaxed!(dmb_cp15!(), ""),
            (Acquire, Acquire | SeqCst) => cmpxchg_store_relaxed!(dmb_cp15!(), dmb_cp15!()),
            (Release, Relaxed) => cmpxchg_release!(""),
            (Release, Acquire | SeqCst) => cmpxchg_release!(dmb_cp15!()),
            // AcqRel and SeqCst compare_exchange are equivalent.
            (AcqRel | SeqCst, Relaxed) => cmpxchg_acqrel!(""),
            (AcqRel | SeqCst, _) => cmpxchg_acqrel!(dmb_cp15!()),
            _ => unreachable!(),
        }
        crate::utils::assert_unchecked(r == 0 || r == 1); // may help remove extra test
                                                          // 0 if the store was successful, 1 if no store was performed
...
#[inline]
unsafe fn atomic_swap_v6(dst: *mut u64, val: u64, order: Ordering) -> u64 {
    debug_assert!(dst as usize % 8 == 0);
    debug_assert_v6!();
    let val = U64 { whole: val };
    // SAFETY: the caller must uphold the safety contract.
    unsafe {
        let (mut prev_lo, mut prev_hi);
        macro_rules! atomic_swap_non_relaxed {
            ($acquire:expr, $release:expr) => {
                asm!(
                    ".arch armv6t2",
                    $release, // release fence
        ...
        match order {
            ...
            Ordering::Acquire => atomic_swap_non_relaxed!(dmb_cp15!(), ""),
            Ordering::Release => atomic_swap_non_relaxed!("", dmb_cp15!()),
            // AcqRel and SeqCst RMWs are equivalent.
            Ordering::AcqRel | Ordering::SeqCst => {
                atomic_swap_non_relaxed!(dmb_cp15!(), dmb_cp15!())
            }
            _ => unreachable!(),
        }
        U64 { pair: Pair { lo: prev_lo, hi: prev_hi } }.whole
...
macro_rules! atomic_rmw_ll_sc_3 {
    ($name_v7:ident, $name_v6:ident, $($op:tt)*) => {
        #[inline]
        ...
        #[inline]
        unsafe fn $name_v6(dst: *mut u64, val: u64, order: Ordering) -> u64 {
            debug_assert!(dst as usize % 8 == 0);
            debug_assert_v6!();
            let val = U64 { whole: val };
            // SAFETY: the caller must uphold the safety contract.
            unsafe {
                ...
                match order {
                    ...
                    Ordering::Acquire => op_non_relaxed!(dmb_cp15!(), ""),
                    Ordering::Release => op_non_relaxed!("", dmb_cp15!()),
                    // AcqRel and SeqCst RMWs are equivalent.
                    Ordering::AcqRel | Ordering::SeqCst => {
                        op_non_relaxed!(dmb_cp15!(), dmb_cp15!())
                    }
                    _ => unreachable!(),
                }
                U64 { pair: Pair { lo: prev_lo, hi: prev_hi } }.whole
...
macro_rules! atomic_rmw_ll_sc_2 {
    ($name_v7:ident, $name_v6:ident, [$($reg:tt)*], $($op:tt)*) => {
        #[inline]
        ...
        #[inline]
        unsafe fn $name_v6(dst: *mut u64, order: Ordering) -> u64 {
            debug_assert!(dst as usize % 8 == 0);
            debug_assert_v6!();
            // SAFETY: the caller must uphold the safety contract.
            unsafe {
...
atomic_rmw_ll_sc_3! {
    atomic_add_v7, atomic_add_v6,
    "adds r2, r4, {val_lo}",
    "adc r3, r5, {val_hi}",
}
atomic_rmw_ll_sc_3! {
    atomic_sub_v7, atomic_sub_v6,
    "subs r2, r4, {val_lo}",
    "sbc r3, r5, {val_hi}",
}
atomic_rmw_ll_sc_3! {
    atomic_and_v7, atomic_and_v6,
    "and r2, r4, {val_lo}",
    "and r3, r5, {val_hi}",
}
atomic_rmw_ll_sc_3! {
    atomic_nand_v7, atomic_nand_v6,
    "and r2, r4, {val_lo}",
    "and r3, r5, {val_hi}",
    "mvn r2, r2",
    "mvn r3, r3",
...
atomic_rmw_ll_sc_3! {
    atomic_or_v7, atomic_or_v6,
    "orr r2, r4, {val_lo}",
    "orr r3, r5, {val_hi}",
}
atomic_rmw_ll_sc_3! {
    atomic_xor_v7, atomic_xor_v6,
    "eor r2, r4, {val_lo}",
    "eor r3, r5, {val_hi}",
}

atomic_rmw_ll_sc_2! {
    atomic_not_v7, atomic_not_v6, [],
    "mvn r2, r4",
    "mvn r3, r5",
}
atomic_rmw_ll_sc_2! {
    atomic_neg_v7, atomic_neg_v6, [],
    "rsbs r2, r4, #0",
    "rsc r3, r5, #0",
}

atomic_rmw_ll_sc_3! {
    atomic_max_v7, atomic_max_v6,
    "mov r3, {val_hi}",
    "subs {r}, {val_lo}, r4",
    "sbcs {r}, {val_hi}, r5",
    "mov {r}, #0",
    ...
    (
        ...
        unsafe fn $name($($arg)*, order: Ordering) $(-> $ret_ty)? {
            fn_alias! {
                ...
                // TODO: v6 and v7 relaxed RMWs are the same.
                ...
                v6_relaxed_fn = $v6_fn(Ordering::Relaxed);
                v6_acquire_fn = $v6_fn(Ordering::Acquire);
                v6_release_fn = $v6_fn(Ordering::Release);
                // AcqRel and SeqCst RMWs are equivalent.
                v6_seqcst_fn = $v6_fn(Ordering::SeqCst);
            }
            #[inline(never)]
