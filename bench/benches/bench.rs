...
#[allow(dead_code, unused_imports)]
#[path = "imp/seqlock64_futex32.rs"]
mod seqlock64_futex32_fallback;
...
fn bench_concurrent_load<A: AtomicInt<T>, T: Copy + From<u32>>(threads: usize) -> A {
    assert!(threads >= 2);
...
fn bench_concurrent_load_store<A: AtomicInt<T>, T: Copy + From<u32>>(threads: usize) -> A {
    assert!(threads >= 2 && threads % 2 == 0);
    let a = black_box(A::new(T::from(1)));
    let barrier = Barrier::new(threads);
    thread::scope(|s| {
...
fn bench_concurrent_store<A: AtomicInt<T>, T: Copy + From<u32>>(threads: usize) -> A {
    assert!(threads >= 2);
    let a = black_box(A::new(T::from(1)));
    let barrier = Barrier::new(threads);
    thread::scope(|s| {
...
fn bench_concurrent_swap<A: AtomicInt<T>, T: Copy + From<u32>>(threads: usize) -> A {
    assert!(threads >= 2);
    let a = black_box(A::new(T::from(1)));
    let barrier = Barrier::new(threads);
    thread::scope(|s| {
...
fn bench_concurrent_store_swap<A: AtomicInt<T>, T: Copy + From<u32>>(threads: usize) -> A {
    assert!(threads >= 2 && threads % 2 == 0);
    let a = black_box(A::new(T::from(1)));
    let barrier = Barrier::new(threads);
    thread::scope(|s| {
...
fn bench_concurrent_fetch_add<A: AtomicInt<T>, T: Copy + From<u32>>(threads: usize) -> A {
    assert!(threads >= 2);
    let a = black_box(A::new(T::from(1)));
    let barrier = Barrier::new(threads);
    thread::scope(|s| {
        ...
        pub(crate) fn $name(c: &mut Criterion) {
            ...
            for threads in set {
                g.bench_function(
                    ...
                    |b| {
                        b.iter(|| bench_concurrent_load::<A, $int_type>(threads));
                    },
                );
                g.bench_function(
                    &format!(
                        "{}_concurrent_load{1}_store{1}_threads",
                        stringify!($int_type),
                        threads / 2
                    ),
                    |b| {
                        b.iter(|| bench_concurrent_load_store::<A, $int_type>(threads));
                    },
                );
                g.bench_function(
                    &format!("{}_concurrent_store{threads}_threads", stringify!($int_type)),
                    |b| {
                        b.iter(|| bench_concurrent_store::<A, $int_type>(threads));
                    },
                );
                g.bench_function(
                    &format!("{}_concurrent_swap{threads}_threads", stringify!($int_type)),
                    |b| {
                        b.iter(|| bench_concurrent_swap::<A, $int_type>(threads));
                    },
                );
                g.bench_function(
                    &format!(
                        "{}_concurrent_store{1}swap{1}_threads",
                        stringify!($int_type),
                        threads / 2
                    ),
                    |b| {
                        b.iter(|| bench_concurrent_store_swap::<A, $int_type>(threads));
                    },
                );
                g.bench_function(
                    &format!("{}_concurrent_fetch_add{threads}_threads", stringify!($int_type)),
                    |b| {
                        b.iter(|| bench_concurrent_fetch_add::<A, $int_type>(threads));
                    },
                );
            }
        }
...
mod bench {
    ...
    impl_atomic!(seqlock64_futex32_fallback::AtomicU128, u128);
    impl_atomic!(mutex_fallback::AtomicU128, u128);
    ...
    benches!(
        ...
        seqlock64_futex32_fallback::AtomicU128,
        u128
    );
    benches!(bench_portable_atomic_mutex_fallback, mutex_fallback::AtomicU128, u128);
    ...
    criterion_group!(
        ...
        bench_portable_atomic_seqlock64_futex32_fallback,
        bench_portable_atomic_mutex_fallback,
...
mod bench {
    ...
    impl_atomic!(mutex_fallback::AtomicU64, u64);
    ...
    benches!(bench_portable_atomic_mutex_fallback, mutex_fallback::AtomicU64, u64);
