...
default_targets=(
  ...
  # TODO: remove once enabled in matrix.sh
  ...
  # mips64-unknown-linux-gnuabin32 # TODO(mips): not in rustc
  # mips64el-unknown-linux-gnuabin32 # TODO(mips): not in rustc
  # mipsisa64r6-unknown-linux-gnuabin32 # TODO(mips): not in rustc
  # mipsisa64r6el-unknown-linux-gnuabin32 # TODO(mips): not in rustc
...
if [[ "${rustc_version}" =~ nightly|dev ]]; then
  ...
  if [[ "${rustc_minor_version}" -ge 98 ]] && [[ -z "${RUSTC:-}" ]] && [[ -n "${TESTS:-}" ]] && [[ -z "${TARGET_GROUP:-}" ]]; then
    subcmd=clippy
...
build() {
  ...
  case "${target}" in
    # TODO: posix PI futex
    *-linux* | *-android* | *-freebsd* | *-fuchsia*)
      RUSTFLAGS="${target_rustflags}" \
        x_cargo "${args[@]}" --features pi-fallback "$@"
      ;;
  esac
  ...
  case "${target}" in
    ...
    i?86-*-linux-*)
      # no libc code path
      CARGO_TARGET_DIR="${target_dir}/asm_syscall" \
        RUSTFLAGS="${target_rustflags} --cfg portable_atomic_test_asm_syscall" \
        x_cargo "${args[@]}" "$@"
      ;;
    aarch64* | arm64*)
