...
if [[ "${rustc_version}" == *"nightly"* ]] || [[ "${rustc_version}" == *"dev"* ]]; then
    ...
    case "${rustc_version}" in
        ...
        *)
            ...
            check_cfg="-Z unstable-options --check-cfg=names(docsrs,portable_atomic_unsafe_assume_single_core,portable_atomic_arm_swp_lock,$(IFS=',' && echo "${build_script_cfg[*]}")) --check-cfg=values(target_pointer_width,\"128\")"
        ...
        *-none* | avr-* | *-esp-espidf)
            ...
            if ! grep <<<"${cfgs}" -q "target_has_atomic=" && [[ -n "${has_asm}" ]]; then
                case "${target}" in
                    bpf*) ;; # TODO
                    thumbv4t-*)
                        RUSTFLAGS="${target_rustflags} --cfg portable_atomic_arm_swp_lock" \
