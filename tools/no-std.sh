...
default_targets=(
  ...
  # l4re
  # aarch64-unknown-l4re-uclibc # TODO: version `GLIBC_2.42' not found
)
  ...
  case "${target}" in
    ...
    aarch64*-l4re*)
      ...
      if [[ "${subcmd}" != "build" ]]; then
        ...
        (
          cd -- "${L4RE_TMPDIR}"
          [[ -e l4re_hello-2_arm_virt.elf ]] || retry curl --proto '=https' --tlsv1.2 -fsSL --retry 10 -O https://l4re.org/download/snapshots/pre-built-images/arm64/l4re_hello-2_arm_virt.elf
