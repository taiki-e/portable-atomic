...
run() {
    ...
    case "${target}" in
        ...
        msp430*)
            ...
            target_rustflags+=" -C link-arg=-lmul_none"
    ...
    (
        ...
        case "${target}" in
            ...
            msp430*)
                ...
                # bool is covered by the run with the default feature.
                # i8 and u8 can be tested at the same time because small.
                # TODO: floats are skipped due to size (region `ROM' overflowed by 19416 bytes).
                # NB: Sync feature list with tests/msp430/Cargo.toml
                for feature in ptr isize usize i8,u8 i16 u16 i32 u32 i64 u64 i128 u128; do
                    # CARGO_TARGET_DIR="${target_dir}/no-std-test" \
                    #     RUSTFLAGS="${target_rustflags}" \
